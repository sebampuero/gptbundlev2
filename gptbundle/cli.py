...
def list_chats():
    try:
        ...
        for chat in chats:
            ...
            table.add_row(
                chat.chat_id, chat.user_email, formatted_ts, str(len(chat.messages))
            )
...
def delete_chat(
    ...
    try:
        ...
        if success:
            ...
            console.print(
                f"[yellow]Chat with ID {chat_id} and timestamp "
                f"{timestamp} not found.[/yellow]"
            )
    except Exception as e:
...
def delete_chats_by_email(
    email: str = typer.Argument(
        ..., help="The email of the user whose chats should be deleted"
    ),
):
    try:
        ...
        console.print(
            f"[green]Successfully deleted {count} chats for email:[/green] {email}"
        )
    except Exception as e:
